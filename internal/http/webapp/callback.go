...
func (m *Module) CallbackOauthGetHandler(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	switch fedihelper.Software(instance.Software) {
	case fedihelper.SoftwareMastodon:
		...
		accessToken, err = m.fedi.Helper(fedihelper.SoftwareMastodon).GetAccessToken(r.Context(), instance, code[0])
		if err != nil {
		...
		accountI, err := m.fedi.Helper(fedihelper.SoftwareMastodon).GetCurrentAccount(r.Context(), instance, accessToken)
		if err != nil {
			l.Errorf("get access token error: %s", err.Error())
			m.returnErrorPage(w, r, nethttp.StatusInternalServerError, err.Error())
