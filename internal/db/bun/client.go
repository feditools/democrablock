...
func sqliteConn(ctx context.Context) (*Bun, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not create sqlite error handler: %s", err)
	}
...
func pgConn(ctx context.Context) (*Bun, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not create postgres error handler: %s", err)
	}
...
func getErrConn(dbConn *bun.DB) (*Bun, error) {
	...
	case dialect.Invalid:
		return nil, fmt.Errorf("invalid dialect %s", dbConn.Dialect().Name())
	case dialect.MSSQL:
		return nil, errors.New("dialect MSSQL unsupported")
	case dialect.MySQL:
		return nil, errors.New("dialect MySQL unsupported")
	case dialect.PG:
		errProc = processPostgresError
	case dialect.SQLite:
		errProc = processSQLiteError
	default:
		return nil, fmt.Errorf("unknown dialect %s", dbConn.Dialect().Name())
	}
